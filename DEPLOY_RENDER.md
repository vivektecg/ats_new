...
- the Node server serves:
  ...
  - `/api/ai/*`
  - `/healthz`
- PostgreSQL is used for ATS records and auth metadata.
- A persistent disk stores uploaded resumes and candidate documents.
- Backend login uses signed sessions and upgraded password hashes.
...
4. Provide values for optional secret environment variables if you want live AI:
   ...
   - `LLM_API_KEY`
   - `LLM_MODEL`
5. Add GitHub backup and Outlook variables if you want those production integrations enabled.
6. Deploy.
...
- `ATS_REQUIRE_AUTH=true` protects ATS data endpoints in production.
- `ATS_SESSION_SECRET` and `ATS_TOKEN_ENCRYPTION_KEY` are generated by Render.
- Uploaded resumes/documents are stored on the `/var/data` persistent disk.
